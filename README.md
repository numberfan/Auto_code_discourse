...
| v11.2 | 强化“主动作优先于最后点名对象”，修正 reasoning/addressee 边界 | **0.725** | 0.918 | 27 | 当前 Macro F1 最佳；`press_for_reasoning`、`challenge` 提升，但 `add_on` 漏检回升 |
| v11.3 | 计划：在保留 reasoning 改善的前提下，专门救回 peer-linked `add_on` recall | - | - | - | 重点修复 `add_on → missed`，避免 Binary F1 继续下降 |
| v12? | 暂不需要；优先沿 v11.x 小步微调 | - | - | - | 当前更适合基于 v11.2 做局部优化，而不是重构框架 |
...
* v11.2：进一步强化“主动作优先于最后点名对象”，修正 `press_for_reasoning -> add_on` 等 addressee 边界后，Macro F1 提升到 **0.725**，成为当前最佳版本；代价是 Binary F1 降到 0.918，说明 peer-linked `add_on` recall 仍需继续补强。
* 下一步（v11.3）：不重构框架，专门修复 `add_on -> missed`，目标是在保住 v11.2 的 reasoning/challenge 改善的同时，把 Binary F1 拉回去。

### 当前判断

* 如果只看 Macro F1，`v11.2` 是当前最佳版本。
* 如果更看重 Binary F1 / trigger 稳定性，`v8.1` 和 `v11.1` 仍然更稳。
* 当前最值得继续迭代的是 `v11.2`，因为它同时具备更强的结构化分析能力和当前最高的 Macro F1。

### v11.3 优化方向

* 重点文件：`prompts/v11/stage1_trigger_addressee.txt`、`prompts/v11/stage1_few_shot.txt`、`prompts/v11/stage3_confirm.txt`
* 重点目标：恢复 peer-linked `add_on` 的 trigger recall，尤其是 `add_on -> missed`
* 重点场景：
  * 教师先简短回应/总结，再邀请另一学生或全班继续
  * related factual question 仍 anchored 在前一学生贡献或 active discussion
  * 多句教师话轮里，前段 explanation 不能吞掉后段 peer-linked invitation
* 暂不继续大改代码框架，优先做 prompt 级小步迭代。
...
* `coded_discourse/evaluation/v11/error_analysis_chris_moon_v1_20260828_102542.json`

* 对当前 v11.2 最有代表性的剩余问题：`add_on -> missed` 仍有 7 次，`press_for_reasoning -> add_on` 降到 1 次，说明下一轮应优先补回 Type B 的 recall，而不是再去大改 Type A。 
